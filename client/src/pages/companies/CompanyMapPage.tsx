...
export function CompanyMapPage() {
  ...
  useEffect(() => {
    ...
    data.forEach(company => {
      ...
      marker.on('click', () => setSelected(company))
      marker.addTo(map)
    })
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] fade-in">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-slate-900">Cartographie clients</h1>
        ...
        {data && (
          <div className="flex items-center gap-4 text-sm">
            <span className="flex items-center gap-1.5 text-slate-600">
              ...
              <strong className="text-slate-900">{data.length}</strong> entreprise{data.length > 1 ? 's' : ''}
            </span>
            <span className="flex items-center gap-1.5 text-slate-600">
              <Ticket className="w-4 h-4 text-red-400" />
              <strong className="text-slate-900">{withTickets}</strong> avec tickets ouverts
            </span>
            <span className="flex items-center gap-1.5 text-slate-600">
              <MapPin className="w-4 h-4 text-slate-400" />
              <strong className="text-slate-900">{sectors.length}</strong> secteur{sectors.length > 1 ? 's' : ''}
            </span>
          </div>
        )}
      </div>
            ...
            <div className="p-4 border-b border-slate-100 space-y-3">
              ...
              <div className="flex items-center gap-2">
                <span
                  ...
                  style={{ backgroundColor: sectorColor(selected.sector) }}
                >
                  {selected.sector}
                </span>
              </div>
              ...
              <button
                onClick={() => navigate(`/companies/${selected.id}`)}
            ...
            <div className="pt-3 border-t border-slate-100 space-y-1">
              ...
              {data?.map(c => (
                ...
                >
                  ...
                  {c._count.tickets > 0 && (
                    <span className="text-[10px] font-bold text-red-500 flex-shrink-0">{c._count.tickets}</span>
                  )}
                </button>
              ))}
            </div>
