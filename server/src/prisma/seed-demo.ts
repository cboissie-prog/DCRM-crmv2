/**
 * seed-demo.ts — Données de démonstration (non destiné à la production).
 * Exécute d'abord le seed de base (permissions + rôles + admin),
 * puis insère les users de test et toutes les données de démo.
 */
...
async function main() {
  ...
  const commercial1 = await prisma.user.upsert({
    ...
    update: { roleId: roleIdByName['COMMERCIAL'] },
    create: {
      email: 'jean.dupont@crm.local',
      password: await bcrypt.hash('test123', 12),
      firstName: 'Jean',
      lastName: 'Dupont',
      phone: '06 23 45 67 89',
      role: 'COMMERCIAL',
      roleId: roleIdByName['COMMERCIAL'],
    },
  })
  const technicien1 = await prisma.user.upsert({
    where: { email: 'alex@crm.local' },
    update: { roleId: roleIdByName['TECHNICIEN'] },
    create: {
      ...
      role: 'TECHNICIEN',
      roleId: roleIdByName['TECHNICIEN'],
    },
  })
  ...
  // Récupérer l'admin pour les données de démo
  const admin = await prisma.user.findFirstOrThrow({ where: { role: 'ADMIN' } })

  console.log('✅ Users de démo créés')
  ...
  // Éviter les warnings "variable non utilisée" sur les contrats non référencés
  void ctr4; void ctr5
  ...
  // Éviter les warnings "variable non utilisée"
  void eq2; void eq4
  ...
  await prisma.ticketComment.createMany({ data: [
    ...
    { ticketId: tkt1.id, content: 'Vérification effectuée chez le client. Le routeur présente des signes de défaillance. Remplacement nécessaire.', isInternal: true, authorName: 'Marie Bernard' },
  ]})
