 ...
 *
 * Pilotage 100 % mensuel : les périodes acceptées sont "AAAA-MM" (et "AAAA"
 * pour la cible annuelle d'entreprise). Le format trimestriel "AAAA-Qn" est refusé.
 *
 ...
 * - allocatedTarget = somme des objectifs individuels du/des mois couverts, sans
 *   double comptage de périmètre (l'objectif global d'un commercial prime sur ses
 *   objectifs par pipeline).
 *
...
describe("Objectifs d'entreprise — /api/targets/company", () => {
  beforeAll(async () => {
    ...
    // Août : 40 000 (Alice) ; juillet : 20 000 (Bob) ; novembre : 15 000 (Alice)
    // + une ouverte (jamais comptée)
    const opps = await Promise.all([
      prisma.opportunity.create({ data: { title: 'CT won A août', value: 40000, stage: 'WON', pipelineId, assignedToId: userAId, closedAt: new Date(2098, 7, 15) } }),
      prisma.opportunity.create({ data: { title: 'CT won B juil', value: 20000, stage: 'WON', pipelineId, assignedToId: userBId, closedAt: new Date(2098, 6, 1) } }),
      prisma.opportunity.create({ data: { title: 'CT won A nov',  value: 15000, stage: 'WON', pipelineId, assignedToId: userAId, closedAt: new Date(2098, 10, 1) } }),
    ...
    // ── Objectifs individuels mensuels ──────────────────────────────────────
    // Alice, août : global 50 000 (prime sur sa ventilation pipeline de 20 000)
    // Alice, juillet : global 10 000 ; Bob, juillet : global 8 000
    await prisma.salesTarget.createMany({
      data: [
        { userId: userAId, period: '2098-08', target: 50000, pipelineId: null },
        { userId: userAId, period: '2098-08', target: 20000, pipelineId },
        { userId: userAId, period: '2098-07', target: 10000, pipelineId: null },
  ...
  it('un COMMERCIAL peut lire (company_targets:read) mais pas écrire → 403', async () => {
    ...
    const getRes = await request(app)
      .get('/api/targets/company?period=2098-08')
    ...
    const postRes = await request(app)
      ...
      .set('Authorization', `Bearer ${accessToken}`)
      .send({ period: '2098-08', target: 100000 })
  ...
  it('POST crée un objectif global, puis upsert sur la même période (pas de doublon)', async () => {
    const res1 = await request(app)
      .post('/api/targets/company')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ period: '2098-08', target: 400000 })
    ...
    const res2 = await request(app)
      .post('/api/targets/company')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ period: '2098-08', target: 500000 })
    expect(res2.status).toBe(201)
    expect(res2.body.data.id).toBe(res1.body.data.id)

    const rows = await prisma.companyTarget.findMany({ where: { period: '2098-08', pipelineId: null } })
    expect(rows).toHaveLength(1)
    expect(rows[0].target).toBe(500000)
  })

  // ── Le format trimestriel n'existe plus ──────────────────────────────────

  it('POST refuse une période trimestrielle ou invalide → 400', async () => {
    for (const period of ['2098-Q3', '2098-13', 'n-importe-quoi']) {
      const res = await request(app)
        .post('/api/targets/company')
        .set('Authorization', `Bearer ${adminToken}`)
        .send({ period, target: 1000 })
      expect(res.status, `period=${period}`).toBe(400)
    }
  })

  it('GET refuse une période trimestrielle ou mal formée → 400', async () => {
    for (const period of ['2098-Q3', 'n-importe-quoi']) {
      const res = await request(app)
        .get(`/api/targets/company?period=${period}`)
        .set('Authorization', `Bearer ${adminToken}`)
      expect(res.status, `period=${period}`).toBe(400)
    }
  })

  it('les objectifs individuels refusent aussi le format trimestriel → 400', async () => {
    const post = await request(app)
      .post('/api/targets')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ userId: userAId, period: '2098-Q3', target: 10000 })
    expect(post.status).toBe(400)

    const get = await request(app)
      .get('/api/targets?period=2098-Q3')
      .set('Authorization', `Bearer ${adminToken}`)
  ...
  it('GET mois : réalisé = CA gagné de tous sur le mois, répartition sans double comptage', async () => {
    const res = await request(app)
      .get('/api/targets/company?period=2098-08')
      .set('Authorization', `Bearer ${adminToken}`)
    ...
    // Seule l'opportunité d'août compte (celles de juillet/novembre et l'ouverte sont exclues)
    expect(global.computedActual).toBe(40000)
    // Alice : global août 50 000 (sa ventilation pipeline de 20 000 ne compte pas) ; Bob : rien en août
    expect(global.allocatedTarget).toBe(50000)
  })

  it("GET année : agrège les 12 mois de l'année", async () => {
    await request(app)
      .post('/api/targets/company')
      .set('Authorization', `Bearer ${adminToken}`)
    ...
    // Toutes les gagnées de l'année : 40 000 (août) + 20 000 (juillet) + 15 000 (novembre)
    expect(global.computedActual).toBe(75000)
    // Alice août 50 000 + Alice juillet 10 000 + Bob juillet 8 000
    expect(global.allocatedTarget).toBe(68000)
  })
  ...
  it('GET objectif ventilé par pipeline : réalisé et répartition limités au pipeline', async () => {
    await request(app)
      .post('/api/targets/company')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ period: '2098-08', target: 100000, pipelineId })
    ...
    expect(scoped.computedActual).toBe(40000) // la gagnée d'août est sur ce pipeline
    // Seule la ventilation pipeline d'Alice compte (20 000) — son global est hors périmètre
